...
def sync_symbols(session: Session, tickers: list[StockSymbol]) -> SymbolSyncResult:
    """Symbol 정보를 DB에 저장

    Args:
        session(Session): DB Session
        tickers(list[StockSymbol]): 불러온 ticker list
...
def upsert_symbols(session: Session, tickers: list[StockSymbol]) -> int:
    return sync_symbols(session, tickers).upserted_count


def _to_payload(ticker: StockSymbol) -> dict[str, object]:
    """SQL문에 사용할 dict 자료형으로 변환합니다.

    Args:
        ticker (StockSymbol): 주식 정보
