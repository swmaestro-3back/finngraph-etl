...
클러스터는 첫 기사 발행일로부터 window_days 동안만 자란다. 새 기사는 시드의
first_published_at 이후 window_days 안에 발행된 것만 합류하고, 배치 안의 새 기사끼리도 발행일
폭이 window_days 를 넘는 병합은 하지 않는다 — 첫 검색으로 몇 달치 기사가 한 배치에 들어와도
같은 사건이 시기별로 갈라진다.
"""
...
class ClusterSeed:
    ...
    # 클러스터 첫 기사의 보도 시각. 이 시각 이후 window_days 안의 기사만 합류한다.
...
def assign_batch(
    ...
    window_days: 클러스터의 발행일 폭 상한. 시드는 first_published_at 이 시작점이다.
    """
    ...
    groups = agglomerative(
        ...
        spans=[(epoch, epoch) for epoch in seed_epochs + doc_epochs],
        max_span=window_days * 86400.0,
    )
